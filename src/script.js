...
class WeatherService {
  constructor(apiKey) {
    this.apiKey = "1d65a2e8eea59954171b44806d18c622";
...
const themeManager = {
  ...
  toggleTheme: function () {
    ...
    localStorage.setItem(
      "theme",
      this.body.classList.contains("dark") ? "dark" : "light"
    );
  },
...
const timeManager = {
  ...
  printLocalDateString: function (data, dateObject) {
    ...
    const localTimeString = this.convertUnixToTimezone(
      dateObject,
      data.timezone
    ).toLocaleString([], {
      hour: "2-digit",
  ...
  displaySunsetSunriseTime: function (
    data,
    localDateObject,
    sunriseTime,
    sunsetTime
  ...
  changeSceneryImage: function (
    ...
    const sunriseHour = this.convertUnixToTimezone(
      sunriseTime,
      data.timezone
    ).getHours();
    const sunsetHour = this.convertUnixToTimezone(
      sunsetTime,
      data.timezone
    ).getHours();

    if (
      this.convertUnixToTimezone(localDateObject, data.timezone).getHours() <
        sunriseHour ||
      this.convertUnixToTimezone(localDateObject, data.timezone).getHours() >=
        sunsetHour
    ) {
...
const selectedLocationWeather = {
  ...
  toggleTemp: function (event) {
    ...
    weatherService.displaySelectedLocationWeather(
      this.locationHeading.textContent
    );
  ...
  displayCurrentTemperature: function (response) {
    if (response.status == 200) {
      ...
      timeManager.displaySunsetSunriseTime(
        data,
        localDateObject,
        apiSunrise,
        apiSunset
...
const globalWeather = {
  ...
  initialize: function () {
    this.globalContainer.addEventListener(
      "click",
